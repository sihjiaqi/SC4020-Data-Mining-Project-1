...
import torch.nn as nn
import torch.nn.functional as F
from torch_scatter import scatter_add  # comes with PyG
...
class RelationalGINConv(nn.Module):
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor, edge_type: torch.Tensor) -> torch.Tensor:
        ...
        m = x[src] * r                                   # [E, d]
        aggr = scatter_add(m, dst, dim=0, dim_size=x.size(0))  # [N, d]
...
res_distmult = train_linkpred_typed(
    ...
    epochs=100, lr=1e-3, weight_decay=1e-4, patience=5,
    show_tqdm=True,
    save_best_path=f"checkpoints/embed_rel/{dataset}/best_rgin_distmult_em={emb_dim}_mlp={hidden_layers}_ag={num_layers}_ds={dataset}.pt",
...
res_dot = train_linkpred_typed(
    ...
    epochs=100, lr=1e-3, weight_decay=1e-4, patience=5,
    show_tqdm=True,
    save_best_path=f"checkpoints/embed_rel/{dataset}/best_rgin_dot_em={emb_dim}_mlp={hidden_layers}_ag={num_layers}_ds={dataset}.pt",
    save_on_improve=True,
...
print_comparison_report(
    ...
    save_path=f"results/embed_rel/{dataset}/comp_rgin_dist_v_dot_em={emb_dim}_mlp={hidden_layers}_ag={num_layers}_ds={dataset}.txt"
)
...
print_comparison_report(
    ...
    right_name="R-GIN + DotProduct",
    right_result=res_dot,
    save_path=f"results/embed_rel/{dataset}/comp_rgin_dist_vs_dot_em={emb_dim}_mlp={hidden_layers}_ag={num_layers}_ds={dataset}.txt"
)
...
res_distmult = train_linkpred_typed(
    enc_dm, dec_dm, dm_typed,
    epochs=500, lr=1e-3, weight_decay=1e-4, patience=80,
    show_tqdm=True,
    save_best_path=f"checkpoints/embed_rel/{dataset}/best_rgin_distmult_em={emb_dim}_mlp={hidden_layers}_ag={num_layers}_ds={dataset}.pt",
    save_on_improve=True,
    hparams={"dataset": dataset, "decoder": "DistMult", "emb_dim": emb_dim, "num_layers": num_layers, "hidden_layers": hidden_layers}
...
res_dot = train_linkpred_typed(
    enc_dot, dec_dot, dm_typed,
    epochs=100, lr=1e-3, weight_decay=1e-4, patience=90,
    show_tqdm=True,
    save_best_path=f"checkpoints/embed_rel/{dataset}/best_rgin_dot_em={emb_dim}_mlp={hidden_layers}_ag={num_layers}_ds={dataset}.pt",
    save_on_improve=True,
    hparams={"dataset": dataset, "decoder": "DotProduct", "emb_dim": emb_dim, "num_layers": num_layers, "hidden_layers": hidden_layers}
